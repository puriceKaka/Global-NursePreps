...
const tabs = [
  ...
  ['register-customer', 'Register customer', UserPlus],
  ['register-phone', 'Register phone', Phone],
  ['register-bike', 'Register bike', Bike],
        ...
        <View style={styles.main}>
          ...
          <View style={styles.pageHeader}>
            <View style={{ minWidth: 0 }}>
              ...
              <Text style={styles.pageSubtitle}>Register customers, phones, and bikes, track follow-up, and view commissions from the centralized CRM.</Text>
            </View>
            <Button icon={RefreshCw} variant="secondary" onPress={loadPortal}>Refresh</Button>
          </View>

          {activeTab === 'dashboard' && <DashboardTab {...props} />}
          {['register-customer', 'register-phone', 'register-bike'].includes(activeTab) && (
            <RegisterTab
              {...props}
              initialProductType={activeTab === 'register-phone' ? 'phone' : 'bike'}
              title={
                activeTab === 'register-phone'
                  ? 'Register customer and phone'
                  : activeTab === 'register-bike'
                    ? 'Register customer and bike'
                    : 'Register customer and product'
              }
            />
          )}
...
function DashboardTab({ portal, onNavigate }) {
  const phoneCount = (portal.products || []).filter((product) => product.productType === 'phone').length;
  const bikeCount = (portal.products || []).filter((product) => product.productType === 'bike').length;
  const assignedPhones = (portal.products || []).filter((product) => product.productType === 'phone' && product.assignedCustomerId).length;
  const assignedBikes = (portal.products || []).filter((product) => product.productType === 'bike' && product.assignedCustomerId).length;
  ...
  return (
    <View style={styles.stack}>
      <View style={styles.statsGrid}>
        ...
        <StatCard label="Phones" value={phoneCount} />
        <StatCard label="Bikes" value={bikeCount} />
        <StatCard label="Assigned phones" value={assignedPhones} />
        <StatCard label="Assigned bikes" value={assignedBikes} />
      </View>
      <View style={styles.twoColumn}>
        <View style={styles.panel}>
          ...
          <View style={styles.quickActionRow}>
            <Button icon={ClipboardList} onPress={() => onNavigate('tasks')} style={styles.quickActionButton}>Open tasks</Button>
            <Button icon={Phone} variant="secondary" onPress={() => onNavigate('register-phone')} style={styles.quickActionButton}>Register phone</Button>
            <Button icon={Bike} variant="secondary" onPress={() => onNavigate('register-bike')} style={styles.quickActionButton}>Register bike</Button>
          </View>
        </View>
        <View style={styles.panel}>
          ...
          />
          <Button icon={UserPlus} onPress={() => onNavigate('register-customer')} style={styles.fullButton}>Register customer</Button>
        </View>
      </View>
    </View>
  );
...
function RegisterTab({ portal, onRefresh, initialProductType = 'bike', title = 'Register customer and product' }) {
  const steps = ['Customer', 'Customer documents', 'Next of kin', 'Kin documents', 'Product'];
  const draftKey = 'bumu.agent.customer-registration-draft.v1';
  ...
  const selectedProductType = String(form.productType || initialProductType || 'bike').toLowerCase() === 'phone' ? 'phone' : 'bike';
  const productLabel = selectedProductType === 'phone' ? 'phone' : 'bike';
  const productTitle = selectedProductType === 'phone' ? 'phone' : 'bike';
  const productIdentifierLabels = selectedProductType === 'phone'
    ? ['IMEI 1', 'IMEI 2']
    ...
    try {
      ...
      if (raw) {
        ...
        if (draft?.form && typeof draft.form === 'object') {
          setForm((current) => ({ ...current, ...draft.form, productType: draft.form.productType || initialProductType || 'bike' }));
        }
        if (Number.isInteger(draft?.step)) {
          setStep(Math.max(0, Math.min(steps.length - 1, draft.step)));
        }
        setMessage('Draft restored. Continue where you left off.');
      }
    } catch {
  ...
  }, [draftLoaded, initialProductType]);

  useEffect(() => {
    if (!draftLoaded) return;

    try {
      const isBlank = Object.entries(form).every(([key, value]) => (
        key === 'productType'
          ? String(value || '').trim().toLowerCase() === String(initialProductType || 'bike').trim().toLowerCase()
          : !String(value || '').trim()
      ));
      ...
      // Draft persistence must not block registration.
    }
  }, [draftLoaded, form, step, initialProductType]);
  ...
  const stepRequirements = [
    ...
    [
      [`Assigned ${productLabel}`, form.productId],
      ['Product model', form.productModel],
      [productIdentifierLabels.join(' / '), form.serialNumber || form.chassisNumber],
  ...
  const assignedProducts = (portal.products || []).filter((product) => (
    String(product.productType || '').toLowerCase() === selectedProductType &&
    !product.assignedCustomerId &&
    ['assigned', 'available'].includes(product.status)
  ));

  function selectProduct(productId) {
    const product = assignedProducts.find((item) => item.id === productId);
    setForm((current) => ({
      ...current,
      productId,
  ...
  function resetForm() {
    setForm((current) => Object.fromEntries(Object.keys(current).map((key) => [key, key === 'productType' ? selectedProductType : ''])));
    setStep(0);
    try {
      window.localStorage.removeItem(draftKey);
    } catch {
  ...
  async function submit() {
    ...
    try {
      ...
      if (result.nextOfKinOtpRequired && result.customer?.id) {
        ...
        setNextOfKinOtp('');
        setMessage(`Application submitted to admin screening. Next-of-kin OTP was sent by SMS. ${promptMessage}`);
        resetForm();
        await onRefresh();
        return;
      }
      ...
      <View style={styles.panel}>
        ...
        <View>
          <Text style={styles.panelTitle}>Next-of-kin acceptance</Text>
            <Text style={styles.panelText}>Use this only if support gives you the manual next-of-kin OTP. The normal flow is for next-of-kin to approve by SMS OTP.</Text>
          </View>
        </View>
        ...
        {step === 4 && (
          <>
            <View style={[styles.field, styles.gridField]}>
              <Text style={styles.label}>Assigned {productLabel}</Text>
              <select
                required
                ...
                style={styles.input}
              >
                <option value="">Choose one of your assigned {productTitle}s</option>
                {assignedProducts.map((product) => (
                  <option key={product.id} value={product.id}>
                    {product.serialNumber || product.chassisNumber} - {product.productModel} ({product.status})
                  </option>
                ))}
              </select>
              {!assignedProducts.length ? (
                <Text style={styles.panelText}>No available {productLabel}s are assigned to this agent. Ask admin to assign stock first.</Text>
              ) : null}
            </View>
            <Field fieldStyle={styles.gridField} label="Product type" value={form.productType} onChangeText={(value) => update('productType', value)} placeholder={productLabel} />
            ...
            <Field fieldStyle={styles.gridField} label={productIdentifierLabels[0]} value={form.serialNumber} onChangeText={(value) => update('serialNumber', value)} placeholder={productIdentifierLabels[0]} />
            <Field fieldStyle={styles.gridField} label={productIdentifierLabels[1]} value={form.chassisNumber} onChangeText={(value) => update('chassisNumber', value)} placeholder={productIdentifierLabels[1]} />
...
const styles = StyleSheet.create({
  ...
  quickActionButton: { flexGrow: 1, flexBasis: 160 },
