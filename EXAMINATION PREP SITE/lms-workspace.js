(() => {
    ...
    function safeResourceUrl(value) {
        ...
        return /^(https?:|data:application\/pdf|data:video\/)/i.test(url) ? url : "";
    }

    async function refreshPrivateMaterialUrls() {
        const session = window.GnpUtils?.getSession?.() || null;
        if (!session?.authToken || !window.GnpSupabase?.signCourseMaterial) return;
        const materials = [
            ...(Array.isArray(course.resources) ? course.resources : []),
            course.uploadedDocument,
            course.lectureVideoAsset
        ].filter(Boolean);
        await Promise.all(materials.map(async (material) => {
            if (!material.bucket || !material.storagePath) return;
            try {
                material.link = await window.GnpSupabase.signCourseMaterial(
                    material.bucket,
                    material.storagePath,
                    session.authToken
                );
            } catch {
                // Keep the last usable signed URL and show the material metadata.
            }
        }));
        if (course.lectureVideoAsset?.link) course.lectureVideo = course.lectureVideoAsset.link;
    }
        ...
        panels.innerHTML = `
            ...
            <section class="lms-structure-panel" data-panel="resources">
                ${objectCards(defaultResources(), "No extra course resources uploaded yet.", (item) => `
                    <article class="lms-structure-card"><span>${escapeHtml(item.type || "Resource")}</span><strong>${escapeHtml(item.title || item.name || "Course resource")}</strong>${safeResourceUrl(item.link) ? `<p><a class="lms-secondary" href="${escapeHtml(safeResourceUrl(item.link))}" target="_blank" rel="noopener">Open learning material</a></p>` : `<p>${escapeHtml(item.link || "Available in this course workspace.")}</p>`}</article>
                `)}
            </section>
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        await refreshPrivateMaterialUrls();
