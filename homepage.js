(() => {
    ...
    function requireSession() {
        ...
        if (!session?.userId || (session.role && session.role !== 'student')) {
    ...
    function renderCourses(userId) {
        ...
        filtered.forEach((course, index) => {
            ...
            startBtn.addEventListener('click', () => {
                ...
                window.location.href = `EXAMINATION%20PREP%20SITE/course-workspace.html?course=${encodeURIComponent(course.id)}`;
            });
    ...
    function refreshCourseSelectors() {
        ...
        const levelSelect = $('#courseLevel');
        const core = window.GnpLearning;
        const catalog = core?.getCourses ? core.getCourses() : core?.COURSE_META || [];

        if (categorySelect) {
            const current = categorySelect.value || 'all';
            const categories = ['all', ...new Set(catalog.map((course) => String(course.category || '').trim()).filter(Boolean))];
            const options = categories.map((category) => `
                <option value="${category === 'all' ? 'all' : category}">${category === 'all' ? 'All categories' : category}</option>
            `).join('');
        ...
        if (levelSelect) {
            ...
            const levels = ['all', ...new Set(catalog.map((course) => String(course.difficulty || '').trim()).filter(Boolean))];
            const options = levels.map((level) => `
                <option value="${level === 'all' ? 'all' : level}">${level === 'all' ? 'All levels' : level}</option>
            `).join('');
