...
-- Import this file into the Supabase SQL editor.
...
-- ---------------------------------------------------------------------------
-- Courses, modules, lectures, and media
-- ---------------------------------------------------------------------------
...
create table if not exists public.courses (
    ...
    lesson_background_image text,
    document_cover_image text,
    lecture_video text,
    lecture_video_name text,
    lecture_video_source text,
...
-- ---------------------------------------------------------------------------
-- Students, progress, payments, support, certificates
-- ---------------------------------------------------------------------------
...
create table if not exists public.exam_attempts (
    ...
    status text not null default 'active' check (
        status in ('active', 'disconnected', 'submitted', 'terminated', 'locked', 'reopened', 'marked', 'published')
    ),
    started_at timestamptz not null default now(),
...
create index if not exists idx_exam_attempts_lecturer_status on public.exam_attempts(lecturer_id, status);

drop trigger if exists trg_student_conduct_updated_at on public.student_conduct;
create trigger trg_student_conduct_updated_at
...
create or replace function public.can_view_course(target_course_id text)
returns boolean
language sql
stable
security definer
...
drop policy if exists "courses_select_public_or_staff" on public.courses;
create policy "courses_select_public_or_staff"
on public.courses
for select
...
drop policy if exists "media_assets_view_public" on public.media_assets;
create policy "media_assets_view_public"
...
drop policy if exists "media_assets_staff_or_owner_insert" on public.media_assets;
create policy "media_assets_staff_or_owner_insert"
on public.media_assets
for insert
with check (
    public.is_staff()
...
drop policy if exists "media_assets_staff_or_owner_update" on public.media_assets;
create policy "media_assets_staff_or_owner_update"
on public.media_assets
for update
using (public.is_staff() or auth.uid() = owner_id)
with check (public.is_staff() or auth.uid() = owner_id);

drop policy if exists "media_assets_staff_or_owner_delete" on public.media_assets;
create policy "media_assets_staff_or_owner_delete"
on public.media_assets
for delete
using (public.is_staff() or auth.uid() = owner_id);
...
drop policy if exists "enrollments_insert_self_or_staff" on public.enrollments;
create policy "enrollments_insert_self_or_staff"
on public.enrollments
for insert
with check (auth.uid() = user_id or public.is_staff());

drop policy if exists "enrollments_update_self_or_staff" on public.enrollments;
create policy "enrollments_update_self_or_staff"
on public.enrollments
for update
using (auth.uid() = user_id or public.is_staff())
with check (auth.uid() = user_id or public.is_staff());
...
drop policy if exists "lesson_progress_select_self_or_staff" on public.lesson_progress;
create policy "lesson_progress_select_self_or_staff"
on public.lesson_progress
for select
using (auth.uid() = user_id or public.is_staff());

drop policy if exists "lesson_progress_self_or_staff" on public.lesson_progress;
create policy "lesson_progress_self_or_staff"
on public.lesson_progress
for all
using (auth.uid() = user_id or public.is_staff())
with check (auth.uid() = user_id or public.is_staff());
...
create policy "payments_insert_self_or_staff"
on public.payments
for insert
with check (auth.uid() = user_id or public.is_staff());
...
drop policy if exists "certificates_select_self_or_staff" on public.certificates;
create policy "certificates_select_self_or_staff"
...
drop policy if exists "support_requests_select_self_or_staff" on public.support_requests;
create policy "support_requests_select_self_or_staff"
...
drop policy if exists "learning_states_select_self_or_staff" on public.learning_states;
create policy "learning_states_select_self_or_staff"
on public.learning_states
for select
using (auth.uid() = user_id or public.is_staff());

drop policy if exists "learning_states_self_or_staff" on public.learning_states;
create policy "learning_states_self_or_staff"
on public.learning_states
for all
using (auth.uid() = user_id or public.is_staff())
with check (auth.uid() = user_id or public.is_staff());

drop policy if exists "student_conduct_select_self_or_staff" on public.student_conduct;
create policy "student_conduct_select_self_or_staff"
on public.student_conduct
for select
using (auth.uid() = student_id or public.is_staff());

drop policy if exists "student_conduct_staff_write" on public.student_conduct;
create policy "student_conduct_staff_write"
on public.student_conduct
for all
using (public.is_staff())
with check (public.is_staff());

drop policy if exists "exam_attempts_select_participant_or_staff" on public.exam_attempts;
create policy "exam_attempts_select_participant_or_staff"
on public.exam_attempts
for select
using (auth.uid() = student_id or auth.uid() = lecturer_id or public.is_staff());

drop policy if exists "exam_attempts_student_insert" on public.exam_attempts;
create policy "exam_attempts_student_insert"
on public.exam_attempts
for insert
with check (auth.uid() = student_id and not exists (
    select 1 from public.student_conduct sc
    where sc.student_id = auth.uid() and sc.status = 'suspended'
));

drop policy if exists "exam_attempts_student_or_staff_update" on public.exam_attempts;
create policy "exam_attempts_student_or_staff_update"
on public.exam_attempts
for update
using (auth.uid() = student_id or auth.uid() = lecturer_id or public.is_staff())
with check (auth.uid() = student_id or auth.uid() = lecturer_id or public.is_staff());
...
values
    ...
    ('course-documents', 'course-documents', false, 52428800, array['application/pdf', 'text/plain', 'text/markdown', 'application/rtf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation']),
...
drop view if exists public.course_public_catalog;

create view public.course_public_catalog as
select
    c.id,
    c.slug,
    c.title,
    ...
    c.content_notes,
    c.uploaded_document,
    c.lesson_background_image,
    c.document_cover_image,
    c.lecture_video,
    c.lecture_video_name,
    c.lecture_video_source,
    c.status,
    c.published_at,
    coalesce(mods.module_count, 0) as live_module_count,
