(() => {
  ...
  const bootstrapFromApi = (() => {
    const keys = [ACCOUNTS_KEY, ERP_KEY, HR_KEY, AUDIT_KEY, NOTIFY_KEY, SMS_OUTBOX_KEY];
    return async () => {
      if (!apiEnabled()) return false;
      const missing = keys.filter((k) => !localStorage.getItem(k));
      if (!missing.length) return true;
      try {
        const res = await fetch(`/api/kv?keys=${encodeURIComponent(missing.join(","))}`);
        if (!res.ok) return false;
        const data = await res.json();
        const items = data?.items && typeof data.items === "object" ? data.items : null;
  ...
  const initNotificationListener = () => {
    ...
    window.addEventListener("storage", safe("dept_notify_storage", (e) => {
      ...
      lastNotifyId = id;
      const role = currentRole();
      const audience = Array.isArray(next?.meta?.audienceRoles) ? next.meta.audienceRoles.map((x) => normalizeRole(x)) : [];
      const allow = role === "director" || !audience.length || audience.includes(role) || audience.includes("all");
      if (!allow) return;
      try {
        sessionStorage.setItem(NOTIFY_SEEN_KEY, id);
  ...
  const initLogin = () => {
    ...
    form.addEventListener("submit", safe("dept_login_submit", (e) => {
      e.preventDefault();
      error.textContent = "";
      error.style.color = "";
  ...
  const initRegister = () => {
    ...
    form.addEventListener("submit", safe("dept_register_submit", (e) => {
      e.preventDefault();
      error.textContent = "";
...
<body>${reportHtml}</body>
  ...
  const initHRPage = () => {
    ...
    renderEmployeesTable();
    window.addEventListener("storage", safe("hr_storage", (e) => {
  ...
  const initFinancePage = () => {
    ...
    render();
    window.addEventListener("storage", safe("finance_storage", (e) => {
  ...
  const initOpsOrSalesPage = (role) => {
    ...
    render();
    window.addEventListener("storage", safe("dept_storage", (e) => {
  ...
  const initAdminPage = () => {
    ...
    render();
    window.addEventListener("storage", safe("admin_storage", (e) => {
      if (e.key !== ERP_KEY) return;
      render();
    }));
  };
