...
const courseImages = {
    ...
    nckArt: courseArt("NCK Licensing", "#0f4c81", "#1fa67a"),
    nclexArt: courseArt("NCLEX Prep", "#293241", "#5b4b8a"),
    lecturesArt: courseArt("Nursing Lectures", "#14679c", "#d7b55d"),
    pharmacologyArt: courseArt("Pharmacology", "#5b4b8a", "#1fa67a"),
    pediatricsArt: courseArt("Pediatrics", "#9b315b", "#d7b55d"),
    medSurgArt: courseArt("Med-Surg", "#2f4858", "#14679c"),
    fundamentalsArt: courseArt("Fundamentals", "#0f4c81", "#1fa67a"),
    assessmentArt: courseArt("Assessment", "#14679c", "#d7b55d"),
    communityArt: courseArt("Community", "#20825f", "#0f4c81"),
    mentalArt: courseArt("Mental Health", "#35605a", "#d7b55d"),
    maternalArt: courseArt("Maternal", "#9b315b", "#14679c"),
    surgicalArt: courseArt("Surgical", "#2f4858", "#d7b55d"),
    leadershipArt: courseArt("Leadership", "#293241", "#1fa67a"),
    geriatricArt: courseArt("Geriatrics", "#6b705c", "#0f4c81"),
    woundArt: courseArt("Wound Care", "#8d3f2d", "#14679c"),
    endocrineArt: courseArt("Endocrine", "#5b4b8a", "#1fa67a")
};
...
const courseCatalog = [
    {
        ...
        summary: "NCK-focused licensing review with priority rules, mock checks, and weak-topic repair.",
        image: courseImages.nckArt,
    ...
    },
    {
        ...
        summary: "NCLEX-RN preparation with NGN cases, CAT-style practice, delegation, and clinical judgment.",
        image: courseImages.nclexArt,
    ...
    },
    {
        ...
        exams: 6,
        format: "Recorded",
        summary: "Short nursing lectures for anatomy, pharmacology, med-surg, pediatrics, and community health.",
        image: courseImages.lecturesArt,
    ...
    },
    {
        ...
        image: courseImages.pharmacologyArt,
    ...
    },
    {
        ...
        image: courseImages.pediatricsArt,
...
const additionalCourseTracks = [
    ["fundamentals-nursing", "Fundamentals of Nursing", "Fundamentals", "Beginner", "Core Skills", 30, 520, 5, "Recorded + Practice", "Patient safety, hygiene, mobility, vital signs, documentation, communication, and bedside nursing foundations.", courseImages.fundamentalsArt, ["basic care", "vitals", "documentation"]],
    ["health-assessment", "Health Assessment Mastery", "Assessment", "Beginner", "Assessment Lab", 34, 620, 6, "Recorded + Skills Lab", "Head-to-toe assessment, history taking, vital signs, focused assessment, and clinical reporting.", courseImages.assessmentArt, ["assessment", "history", "vitals"]],
    ...
    ["community-health", "Community Health Nursing", "Community Health", "Intermediate", "Public Health", 28, 430, 4, "Recorded + Cases", "Health promotion, epidemiology, family health, disease prevention, and community assessment.", courseImages.communityArt, ["community", "public health", "prevention"]],
    ["mental-health", "Mental Health Nursing Review", "Mental Health", "Intermediate", "Therapeutic Care", 30, 500, 5, "Live Review", "Therapeutic communication, risk assessment, psychiatric conditions, crisis care, and safety planning.", courseImages.mentalArt, ["mental health", "communication", "risk"]],
    ["maternal-newborn", "Maternal and Newborn Nursing", "Maternal Health", "Intermediate", "Mother-Baby", 36, 620, 6, "Live + Recorded", "Pregnancy, labor, postpartum, newborn assessment, emergencies, and parent teaching.", courseImages.maternalArt, ["maternal", "newborn", "postpartum"]],
    ...
    ["surgical-nursing", "Perioperative and Surgical Nursing", "Surgical Nursing", "Intermediate", "Surgery Prep", 26, 410, 4, "Recorded", "Pre-op, intra-op, post-op care, anesthesia safety, wound monitoring, and discharge teaching.", courseImages.surgicalArt, ["surgery", "perioperative", "wound"]],
    ["leadership-management", "Nursing Leadership and Management", "Leadership", "Intermediate", "Management", 24, 360, 4, "Recorded", "Delegation, supervision, ethics, team communication, patient safety, and quality improvement.", courseImages.leadershipArt, ["leadership", "delegation", "management"]],
    ...
    ["geriatric-nursing", "Geriatric Nursing Care", "Geriatrics", "Intermediate", "Older Adult", 28, 440, 4, "Cases", "Older adult assessment, falls, dementia care, polypharmacy, mobility, and family support.", courseImages.geriatricArt, ["geriatric", "older adult", "falls"]],
    ...
    ["wound-care", "Wound Care and Tissue Viability", "Wound Care", "Intermediate", "Clinical Skills", 26, 390, 4, "Skills Lab", "Wound assessment, dressing selection, pressure injury prevention, diabetic wounds, and documentation.", courseImages.woundArt, ["wound", "pressure injury", "dressing"]],
    ...
    ["diabetes-endocrine", "Diabetes and Endocrine Nursing", "Endocrine", "Intermediate", "Diabetes Care", 28, 520, 5, "Recorded + Questions", "Diabetes care, insulin safety, endocrine emergencies, patient teaching, and complication prevention.", courseImages.endocrineArt, ["diabetes", "insulin", "endocrine"]],
...
function applyCatalogSessionState(loggedIn) {
    document.body.classList.toggle("has-session", loggedIn);
    ...
    const guestBanner = document.getElementById("guestAccessBanner");
    if (guestBanner) {
...
function initializeCatalogPage() {
    ...
    const loggedIn = hasStudentSession();
    applyCatalogSessionState(loggedIn);
    ...
    if (requestedEnrollment && loggedIn && getCourseById(requestedEnrollment)) {
...
function renderCourseCard(course, state) {
    ...
    const loggedIn = hasStudentSession();
    const percent = loggedIn ? getProgressPercent(course.id, state) : 0;
    ...
    const entryLabel = enrolled ? "Continue learning" : (loggedIn ? "Enroll and start" : "Create account");
        ...
        <article class="course-card homepage-style-course-card reveal-on-scroll${selected ? " selected-course-card" : ""}" data-select-course="${course.id}" tabindex="0">
            ...
            <div class="course-card-body">
                <div class="badge-row">
                    ...
                    <span class="badge alt">${Number(course.price || 0) > 0 ? `KES ${Number(course.price).toLocaleString()}` : "Free preview"}</span>
                </div>
                <h4>${course.title}</h4>
                <p>${course.summary}</p>
                ...
                ${termPreview}
                <div class="muted small">Create an account to enroll, save progress, and open the learning workspace.</div>
                <div class="course-actions card-actions guest-actions">
                    <a class="btn-primary" href="${buildAuthHref("register")}">${entryLabel}</a>
                    <a class="btn-outline" href="${buildAuthHref("login")}">Login</a>
                </div>
            </div>
        </article>
    `;
...
function renderSelectedProgram(course, state) {
    ...
    if (openBtn) {
        if (!loggedIn) {
            openBtn.href = buildAuthHref("register");
            openBtn.removeAttribute("target");
            openBtn.removeAttribute("rel");
            openBtn.textContent = "Create account to enroll";
    ...
    if (enrollBtn) {
        enrollBtn.textContent = enrolled ? "Continue learning" : (loggedIn ? "Enroll Now" : "Create account to enroll");
        enrollBtn.classList.toggle("btn-outline", enrolled);
        enrollBtn.classList.toggle("btn-primary", !enrolled);
    }
    document.getElementById("selected-enrollment-status").textContent = loggedIn ? (enrolled ? "Enrolled" : "Not enrolled") : "Preview only";
    document.getElementById("selected-progress-status").textContent = loggedIn ? `${percent}%` : "Locked";
    document.getElementById("selected-progress-bar").style.width = loggedIn ? `${percent}%` : "0%";
    document.getElementById("selected-last-activity").textContent = loggedIn
        ? (progress.lastVisited
            ? `Last activity: ${formatDate(progress.lastVisited)}`
            : "No activity yet. Start this program to begin saving progress.")
        : "Login to save course progress and continue learning.";
    const guestBanner = document.getElementById("guestAccessBanner");
    if (guestBanner) {
        guestBanner.classList.toggle("hidden", loggedIn);
    }
    const actionRow = document.querySelector("#selectedCourse .lms-actions");
    if (actionRow) {
        actionRow.classList.toggle("hidden", !loggedIn);
    }
